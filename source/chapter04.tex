...
and \gls{dta} will be provided in section \ref{chap: RCPDTAPseudo}. Finally, the 
...
\subsection{Connection establishment based on \gls{tcp} socket}
Compared to WebSocket and RESTful API, the connection established with \gls{tcp} 
socket is relatively simple. Once the \gls{dta} runs, it creates a \gls{tcp} socket 
object in a new thread, waits for the initial "welcome" message from \gls{rcp} 
and then open the socket after confirmation. 
...
The \gls{dta} can run on the same device as \gls{rcp} or a device dedicated 
