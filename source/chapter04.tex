...
the base communication protocol for \gls{mas}. There are several reasons for the choice. 
...
\begin{figure}[htb]
    ...
    \gls{tcp} sockets, agent can be either client or server by creating 
...
communication protocol for \gls{mas}, we now focus on the system decentralization.  
Except for the ideas of the decentralization of tasks for each sub-agent, the agent 
communication system can also be decentralized. For example, reprsented as \gls{ca}, 
server is responsible for handling all connections asynchronously, which should be 
...
be, if an agent receives conflicting messages from different servers, it will probably 
perform incorrect actions. A possible way it to design a mechanism to predefine the 
agent interaction to avoid the system inconsistency. In our use cases, the communication 
...
while the prodution-oriented communications between each \gls{ra} will go through a 
sublevel server. Although having only two servers in our study, the number of servers 
can be expanded according to the scale of the \gls{mas}. For instance, if some agent 
will never access other parts of agents during the production process, a subserver should 
...
respectively, resulting on different communication meachnism. 
...
show the simplified function calls for each conection step from the client side: 
\begin{enumerate}
...
a response indicating whether the function call is sucessful or not. The 
...
\begin{enumerate}
    ...
    \item use \textit{$response = requests.get(f'{server\_address}/get\_messages/{self\_client_name}$)} to GET a message from server
\end{enumerate}

At the server side, functions to process data and give response are defined as:
\begin{enumerate}
