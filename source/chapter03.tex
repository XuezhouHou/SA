...
\State \textbf{Class} messageSender
    ...
    \State \textbf{\qquad \qquad while} recipientNotFound \textbf{do}    
    ...
    \State \qquad {\#similar to $send\_and\_receive()$ but send an image }
    \State \textbf{\qquad function} {$send\_image\_and\_receive(self, recipient, image\_path, priority)$}
    \State \qquad \qquad ...
    % \State \qquad \qquad Establish a WebSocket connection
    % \State \textbf{\qquad \qquad while} recipient not found \textbf{do}   
    % \State \qquad \qquad \qquad Send prioritized json image and wait for response
    
    \State \qquad {\#similar to $send\_capCheck\_and\_receive()$ but raise availibility exceptions } 
...
\State \textbf{Class} messageReceiver
    \State \textbf{\qquad function} {$receive\_and\_send(self, message, priority)$}
    \State \qquad \qquad Establish a WebSocket connection
    \State \textbf{\qquad \qquad while} message not received \textbf{do}
    \State \qquad \qquad \qquad Receive, process and send messages with priority
    \State \qquad {\#similar to $receive\_and\_send()$ but receive and save an image }
    \State \textbf{\qquad function} {$receive\_image\_and\_send(self, message, priority)$}
    \State \qquad \qquad ...    
    % \State \qquad \qquad Establish a WebSocket connection
    % \State \textbf{\qquad \qquad while} message not received \textbf{do}
    % \State \qquad \qquad \qquad Receive, parse json, save images and send messages with priority
    \State \textbf{EndClass}
\State \textbf{Class} agentsAllocation
    ...
    \State \qquad \qquad Create sequence lists for primitives agents allocation    
\State \textbf{EndClass}
\State \textbf{Main:}
\State \qquad Instantiate agentsAllocation, messageSender and messageReceiver
\State \qquad \textbf{repeat}
\State \qquad \qquad $sequences = allocate\_agents\_with\_seq\_primitives(custRequirement)$
...
\begin{algorithm}
    ...
    \State {Initialize} agent relevant variables
    
    \State \textbf{Class} messageSender
    \State \textbf{\qquad function} {$send\_and\_receive\_messages(self, recipient, message, priority)$}
    \State \qquad \qquad 1. Establish a WebSocket connection
    \State \qquad \qquad 2. Send and receive messages
    \State \qquad \qquad 3. Handle exceptions
\State \textbf{EndClass}

\State \textbf{Class} messageReceiver
\State \textbf{\qquad function} {$receive\_and\_send\_messages(self, recipient, message, priority)$}
    \State \qquad \qquad 1. Establish a WebSocket connection
    \State \qquad \qquad 2. Receive, process and send messages
    \State \qquad \qquad 3. Handle exceptions
\State \textbf{EndClass}
   
    \State \textbf{Main:}
    \State {\qquad Instantiate} message sender and receiver
    \State {\qquad Wait} for connectivity check from Coordinator agent
    \State {\qquad} Receive capability check results from coordinator agent
    \State \textbf{\qquad repeat}
    \State \textbf{\qquad \qquad if} {agent is first in sequence} \textbf{then}
        \State {\qquad \qquad \qquad wait} for availability check from Coordinator agent
        \State \textbf{\qquad \qquad else}
        \State \textbf{\qquad \qquad \qquad if} {previous agent is not self} \textbf{then}
            \State {\qquad \qquad \qquad wait} receive availability check and inform message from last agent
