...
\State \textbf{Class} messageSender
    \State \textbf{\qquad procedure} {$send\_and\_receive(self, recipient, message, priority)$}
    ...
    \State \textbf{\qquad procedure} {$send\_capCheck\_and\_receive(self, recipient, primitive, priority)$}
    \State \qquad \qquad Establish a WebSocket connection
    \State \textbf{\qquad \qquad while} recipient not found \textbf{do} 
    \State \qquad \qquad \qquad Send prioritized messages and wait for response
    \State \qquad \qquad \qquad Handle capability exceptions
    \State \qquad {\#similar to $send\_and\_receive()$ but send an image }
    \State \textbf{\qquad procedure} {$send\_image\_and\_receive(self, recipient, image\_path, priority)$}
    \State \qquad \qquad ...
    \State \qquad {\#similar to $send\_capCheck\_and\_receive()$ but raise availibility exceptions } 
    \State \textbf{\qquad procedure} {$send\_availCheck\_and\_receive(self, recipient, primitive, priority)$}
    \State \qquad \qquad ...

\State \textbf{EndClass}
\State \textbf{Class} messageReceiver
    \State \textbf{\qquad procedure} {$receive\_and\_send(self, response, priority)$}
    \State \qquad \qquad Establish a WebSocket connection
    ...
    \State \textbf{\qquad procedure} {$receive\_image\_and\_send(self, response, priority)$}
    \State \qquad \qquad ...    
    \State \textbf{EndClass}
...
\State \textbf{Class} messageReceiver
    \State \textbf{\qquad procedure} {$receive\_and\_send(self, response, serverID)$}
    \State \qquad \qquad Establish a WebSocket connection
    \State \textbf{\qquad \qquad while} message not received \textbf{do}
    \State \qquad \qquad \qquad Receive, process and send responses with priority
    \State \textbf{\qquad procedure} {$receive\_capCheck\_and\_send(self, response, serverID)$}
    \State \qquad \qquad Establish a WebSocket connection
    \State \textbf{\qquad \qquad while} message not received \textbf{do}
    \State \qquad \qquad \qquad Receive, check capability and send responses with priority
    \State \qquad \qquad \qquad Handle capability exceptions
    \State \textbf{\qquad procedure} {$receive\_sequences\_and\_send(self, response, serverID)$}
    \State \qquad \qquad Establish a WebSocket connection
    \State \textbf{\qquad \qquad while} message not received \textbf{do}
    \State \qquad \qquad \qquad Receive, store sequences and send responses with priority

    \State \qquad {\#similar to $receive\_and\_send()$ but receive and save an image }
    \State \textbf{\qquad procedure} {$receive\_image\_and\_send(self, response, serverID)$}
    \State \qquad \qquad ...    
    \State \qquad {\#similar to $receive\_capCheck\_and\_send()$ but check availability}
    \State \textbf{\qquad procedure} {$receive\_availCheck\_and\_send(self, response, serverID)$}
    \State \qquad \qquad ...  
    \State \textbf{EndClass}
...
\begin{algorithm}
    ...
    \State \textbf{Main:}
    \State {\qquad Instantiate} messageSender and messageReceiver
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \State \textbf{\qquad procedure} handler(websocket, path)  
        ...
        \State \textbf{\qquad procedure} $processMessage(recipient, stringMsg)$   
        ...
        \State \textbf{\qquad procedure} $processJsonMessage(recipient, jsonMsg)$   
        \State \textbf \qquad \qquad prioritize messages from priorityDict and store in messageQueue
        \State \textbf \qquad \qquad handle messages with higher priority first 
