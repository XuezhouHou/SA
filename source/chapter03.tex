...
\chapter{Methodology}%
This chapter describes the methodologies of \gls{mas} design. 
The general \gls{mas} is separated as internal (section 3.1) and external(section 3.2) parts,  
with a \gls{mas} for \gls{cda} and \gls{ra} as an internal and \gls{dt} agent as an external system. 
The two systems are decoupled from each other but still still under the concept of agent based operation system. 
\section{Internal}
...
Eventually the methodologies of using python WebSocket and RESTful API are listed in pseudocode. 
