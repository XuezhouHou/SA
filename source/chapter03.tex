...
\begin{breakablealgorithm}
    ...
    \State \textbf{function} {$create\_client\_thread(agentIP)$}
        ...
        \State \qquad \qquad $process\_data(clientSocket)$    
    \State \textbf{function} $process\_data(clientSocket)$
        \State \qquad \qquad {$data = read\_data(clientSocket)$}  
        \State \qquad \qquad {$process\_data\_upload\_and\_download(data)$}  
    \State \textbf{function} {$read\_data(clientSocket)$}
        \State \qquad Establish a \gls{tcp} connection with \gls{ra}    
    \State \textbf{Main:}
        \State \qquad \textbf{for} agentIP in IPs \textbf{do}
        \State \qquad \qquad {$read\_and\_upload\_processData(agentIP)$}
        \State \textbf{End}
    \end{algorithmic}
\end{breakablealgorithm}
