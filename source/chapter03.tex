...
The reason of choosing application layer protocols over transport layer protocols according to table \ref{tab:transportlayer} and \ref{tab:applicationlayer} are listed as following:
\begin{enumerate}
    \item Semantic-rich message patterns 
    \item Higher flexibility
    \item Standardized communication between different different agents
    \item Header that contains agent relevant information
...
\State \textbf{Class} agentsAllocation
    ...
    \State \qquad \qquad 1. Find tasks from customer requirements
    \State \qquad \qquad 2. Breakdown tasks into skills
    \State \qquad \qquad 3. Breakdown skills into primitives
    \State \qquad \qquad 4. Allocate agents
\State \textbf{EndClass}
\State \textbf{Main:}
\State {\qquad Instantiate} allocated agents and message sender and receiver
\State {\qquad}Agent check connections
\State {\qquad}Agent check capabilities
\State {\qquad Notify} agents about finished checks
\State {\qquad}Send sequence and agent information
\State {\qquad Start} availability check to the first agent in sequence
...
\State \textbf{EndClass}
    ...
    \State \textbf{\qquad \qquad if} {agent is first in sequence} \textbf{then}
        \State {\qquad \qquad \qquad wait} receive availability check from Coordinator agent
